...
namespace FastSms
{
    public class FastSmsClient : IFastSmsClient
    {
        ...
        {
            var validityPeriod = 86400;
            if (messageToList.ValidityPeriod != null && messageToList.ValidityPeriod > 0 && messageToList.ValidityPeriod <= 86400)
                validityPeriod = (int)messageToList.ValidityPeriod;
            ...
            var requestUrl = string.Format("{0}{1}&Action=Send&DestinationAddress=List&ListName={2}&SourceAddress={3}&Body={4}&ScheduleDate={5}&SourceTON={6}&ValidityPeriod={7}" +
                ...
                messageToList.ScheduleDate, messageToList.SourceTon, validityPeriod, messageToList.GetAllMessageIDs,
